 ...
 */
public class InstantiatedDomainStoreBackup1 {

  private Map<Class, PrimaryIndex> primaryIndices;

  public InstantiatedDomainStoreBackup1() {
  ...
  public class SecondaryIndex<SK, PK, E> extends com.sleepycat.persist.SecondaryIndex<SK, PK, E> {
    ...
    public E put(PK primaryKey, SK secondaryKey, E entity) throws DatabaseException {
      ...
      if (previous != null) {
        ...
        for (Iterator<Entity> iterator = orderedEntities.iterator(); iterator.hasNext();) {
          Entity secondaryIndexEntity = iterator.next();
//          if (secondaryIndexEntity.primaryKey == null) {
//            Nop.breakpoint();
//          }
          if (secondaryIndexEntity.primaryKey.equals(primaryKey)) {
            iterator.remove();
            break;
          }
        }
      ...
      if (orderedEntities.size() == 0) {
        ...
        orderedIndex = Collections.binarySearch(orderedEntities, secondaryKey, secondaryKeyComparator);
        if (orderedIndex < 0) {
          orderedIndex *= -1;
        }
      }
      Entity secondaryEntity = new Entity(secondaryKey, primaryKey, entity);

      if (orderedIndex == 0 || orderedIndex > orderedEntities.size()) {
        orderedEntities.add(secondaryEntity);
      } else {
        orderedEntities.add(orderedIndex, secondaryEntity);
      }
...
//          return ((SK) o1).compareTo(getSecondaryKey((E) o2));
        } else if (o1 instanceof InstantiatedDomainStoreBackup1.SecondaryIndex.Entity) {
          return SecondaryIndex.this.compare(((Entity) o1).getSecondaryKey(), (SK) o2);
        } else if (o2 instanceof InstantiatedDomainStoreBackup1.SecondaryIndex.Entity) {
    ...
    public List<Entity> orderedEntities = new ArrayList<Entity>();
    ...
    public EntityCursor<E> entities(SK fromKey, boolean fromInclusive, SK toKey, boolean toInclusive) throws DatabaseException {
      ...
      List<Entity> orderedEntities = new ArrayList<Entity>(this.orderedEntities);
