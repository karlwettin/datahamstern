...
import se.datahamstern.event.EventReader;
 ...
 * <p/>
 * This is the same event JSON as produced by {@link se.datahamstern.event.EventQueue#toJSON(se.datahamstern.event.Event)}.
 * <p/>
 ...
 */
public class StreamingJsonEventReader implements EventReader {
   ...
   * @throws Exception
   */
  public StreamingJsonEventReader(Reader reader) throws Exception {
...
//    "version" : "1",
    String eventVersion = assertNextElementValueString("version");
    // todo based on version switch to that parser!

    assertNextAnyOfEvents(JSONStreamReader.Event.NEXT_VALUE);
//    "identity" : "uuid",
    event.setIdentity(assertNextElementValueString("identity"));
